...
# Define UPLOADS_DIR and its subfolders
UPLOADS_DIR = './uploaded_images'
UPLOAD_SUBFOLDERS = {
    "Butterfly Species": os.path.join(UPLOADS_DIR, "species"),
    "Life Stages": os.path.join(UPLOADS_DIR, "stages"),
    "Pupae Defects": os.path.join(UPLOADS_DIR, "pupae"),
    "Larval Diseases": os.path.join(UPLOADS_DIR, "larvae"),
}

# Create upload directories if they don't exist
os.makedirs(UPLOADS_DIR, exist_ok=True)
for subfolder_path in UPLOAD_SUBFOLDERS.values():
...
hostplants_names=list(set(plant for species in SPECIES_HOST_PLANTS.values() for plant in species['plant']))
...
def get_recommended_actions(classification_result, classifier_type):
    ...
    if classifier_type == "Larval Diseases":
        ...
        elif class_name in ["Deformed body", "Overbend", "Stretch abdomen"]: # Group similar defects
        ...
        elif class_name == "Overbend":
            recommendations.append("Monitor closely for emergence. If no emergence after expected period, gently check viability. Note that emergence rate might be lower.")
        elif class_name == "Stretch Abdomen":
            recommendations.append("Monitor closely for emergence. If no emergence after expected period, gently check viability. Note that emergence rate might be lower.")
...
# --- Logging Function ---
def log_classification(image_name, result, classifier_type, csv_file_path):
    ...
    log_entry = {
        ...
        "Health Score": f"{calculate_health_score_and_grade(result, classifier_type)[0]:.2f}%",
        "Quality Grade": calculate_health_score_and_grade(result, classifier_type)[1],
        "Recommendations": "; ".join(get_recommended_actions(result, classifier_type))
    }

    # Add specific details based on classifier type
    if classifier_type == "Butterfly Species":
        log_entry["Scientific Name"] = result.get('scientific_name', 'N/A')
        log_entry["Family"] = result.get('family', 'N/A')
        log_entry["Estimated Value"] = result.get('value', 'N/A')
        log_entry["Host Plant(s)"] = ", ".join(result.get('plant', ['N/A'])) if isinstance(result.get('plant'), list) else result.get('plant', 'N/A')
        log_entry["Daily Consumption (g)"] = result.get('dailyConsumption', 'N/A')
    elif classifier_type == "Life Stages":
        log_entry["Stage Info"] = result.get('stages_info', 'N/A')
    elif classifier_type == "Pupae Defects":
        log_entry["Defect Info"] = result.get('quality_info', 'N/A')
        log_entry["Impact Score"] = result.get('impact_score', 'N/A')
    elif classifier_type == "Larval Diseases":
        log_entry["Treatment Info"] = result.get('treatment_info', 'N/A')
        log_entry["Impact Score"] = result.get('impact_score', 'N/A')
    ...
    st.success(f"Classification logged to {os.path.basename(csv_file_path)}")
...
if selected_menu_item == "Image Classifiers":
    ...
    if uploaded_file is not None:
        ...
        image_name = f"webcam_capture_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
    ...
    if source_image is not None:
        ...
        # Determine the upload subfolder based on classifier_choice
        upload_subfolder = UPLOAD_SUBFOLDERS.get(classifier_choice, UPLOADS_DIR) # Default to main UPLOADS_DIR if not found

        # Save the uploaded/captured image to the designated subfolder
        image_save_path = os.path.join(upload_subfolder, image_name)
        try:
            # For BytesIO objects (from st.file_uploader and st.camera_input)
            with open(image_save_path, "wb") as f:
                f.write(source_image.getbuffer())
            st.info(f"Image saved to: {image_save_path}")
        except Exception as e:
            st.error(f"Error saving image: {e}")
            status_placeholder.empty()
            # It's good practice to return here if image saving fails,
            # as subsequent operations depend on the image being saved.
            st.stop() # Stop execution if critical error

        # Determine which model and data to use based on classifier_choice
        if classifier_choice == "Butterfly Species":
            ...
            csv_file_path = os.path.join(DATA_DIR, 'butterfly_species_classifications.csv')
            ...
            csv_file_path = os.path.join(DATA_DIR, 'lifestages_classifications.csv')
            ...
            csv_file_path = os.path.join(DATA_DIR, 'pupaedefects_classifications.csv')
            ...
            csv_file_path = os.path.join(DATA_DIR, 'larvaldiseases_classifications.csv')

        if model_to_use:
            # Pass the BytesIO object directly to classify_image, as it can handle it
            classification_result = classify_image(source_image, model_to_use, class_names_to_use, details_to_use)
            status_placeholder.empty() # Clear "Classifying..." message
            ...
            if classification_result:
                ...
                for pred in classification_result['top_predictions']:
                    st.write(f"- **{pred['class_name']}**: {pred['score']:.2f}% Confidence")
                st.markdown("</div>", unsafe_allow_html=True)

                # Display primary classification details
                st.markdown("<div class='glasmorphism-card'>", unsafe_allow_html=True)
                st.markdown("<h4>Primary Classification:</h4>", unsafe_allow_html=True)
                st.write(f"**Predicted {classifier_choice}:** {classification_result['class_name']}")
                st.write(f"**Confidence:** {classification_result['score']:.2f}%")

                if classifier_choice == "Butterfly Species":
                    if 'scientific_name' in classification_result:
                        st.write(f"**Scientific Name:** {classification_result['scientific_name']}")
                    if 'family' in classification_result:
                        st.write(f"**Family:** {classification_result['family']}")
                    if 'discovered' in classification_result:
                        st.write(f"**Discovered By:** {classification_result['discovered']} ({classification_result.get('year', 'N/A')})")
                    if 'description' in classification_result:
                        st.write(f"**Description:** {classification_result['description']}")
                    if 'value' in classification_result:
                        st.write(f"**Estimated Value:** ${classification_result['value']:.2f}")
                    if 'plant' in classification_result:
                        plant_name = ", ".join(classification_result['plant']) if isinstance(classification_result['plant'], list) else classification_result['plant']
                        st.write(f"**Host Plant(s):** {plant_name}")
                    if 'dailyConsumption' in classification_result:
                        st.write(f"**Daily Consumption:** {classification_result['dailyConsumption']}g")

                elif classifier_choice == "Life Stages":
                    if 'stages_info' in classification_result:
                        st.write(f"**Stage Information:** {classification_result['stages_info']}")

                elif classifier_choice == "Pupae Defects":
                    if 'quality_info' in classification_result:
                        st.write(f"**Defect Information:** {classification_result['quality_info']}")
                    if 'impact_score' in classification_result:
                        st.write(f"**Impact Score (0-1):** {classification_result['impact_score']:.2f}")

                elif classifier_choice == "Larval Diseases":
                    if 'treatment_info' in classification_result:
                        st.write(f"**Treatment Information:** {classification_result['treatment_info']}")
                    if 'impact_score' in classification_result:
                        st.write(f"**Impact Score (0-1):** {classification_result['impact_score']:.2f}")

                health_score, quality_grade = calculate_health_score_and_grade(classification_result, classifier_choice)
                st.write(f"**Calculated Health Score:** {health_score:.2f}%")
                ...
                if csv_file_path:
                    log_classification(image_name, classification_result, classifier_choice, csv_file_path)
            else:
                st.error("Classification failed. Please try again with a different image or check model files.")
        else:
            st.warning("Selected model is not loaded. Please check the model files.")
