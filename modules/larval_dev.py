...
def larval_stages_app():
    ...
    with tab1:
        ...
        st.write("Upload an image of a larva, or take a photo, to predict its current stage. Prediction requires the Keras model to be present.")
        ...
        # --- ADDED CAMERA AND UPLOAD OPTIONS ---
        ...
        if upload_option == "Upload File":
            ...
            image_file = st.camera_input(
                "Take a Photo of the Larva",
                help="Use your device camera to capture the image."
            )
        # --- END IMAGE SOURCE SELECTION ---
        
        if image_file is not None:
            # Display the selected/captured image
            st.image(image_file, caption='Image for Classification', width='content')
